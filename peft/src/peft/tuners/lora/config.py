...
class LoraConfig(PeftConfig):
    ...
    use_hqt: bool = field(
        ...
        metadata={
            "help": (
                "Use Hadamard Quantized LoRA. This technique quantizes the weights of the base layer using Hadamard "
            )
        },
    )
    hqt_hq_config: Optional[dict] = field(
        default=None,
        metadata={
            "help": (
                "The configuration of Hadamard Quantization. If this is passed, then Hadamard Quantization will be used "
                "to quantize the backbone weights and initialize Lora layers. Also set `init_lora_weights='hqt'` in this case."
            )
        },
    )
